...
coreNames_cortexA32 = ["APSS Cortex-A32"]
coreNames_cortexM55 = ["RTSS-HP Cortex-M55", "RTSS-HE Cortex-M55"]
...
class DtslScript(ConfigurationBaseSDF):
    ...
    def getOptionList():
        return [
            DTSLv1.tabSet("options", "Options", childOptions=
                ...
                +[DTSLv1.tabPage("cortexA32", "Cortex-A32", childOptions=[
                    DTSLv1.enumOption('traceCapture', 'Trace capture method', defaultValue="none",
                        values = [("none", "None"), ("APSS TMC", "System Memory Trace Buffer (APSS TMC/ETR)"), ("SoC TMC", "System Memory Trace Buffer (SoC TMC/ETR)"), DtslScript.getOffChipTraceOption()]),
                    DTSLv1.booleanOption('coreTrace', 'Enable Cortex-A32 core trace', defaultValue=False,
                        childOptions = [
                            DTSLv1.booleanOption('Cortex_A32_0', 'Enable APSS Cortex-A32 trace', defaultValue=True),
                            ...
                            ETMv4TraceSource.cycleAccurateOption(DtslScript.getSourcesForCoreType("Cortex-A32")),
                ...
                +[DTSLv1.tabPage("cortexM55", "Cortex-M55", childOptions=[
                    ...
                    DTSLv1.booleanOption('coreTrace', 'Enable Cortex-M55 core trace', defaultValue=False,
                        childOptions = [
                            DTSLv1.booleanOption('Cortex_M55_0', 'Enable RTSS-HP Cortex-M55 trace', defaultValue=True),
                            DTSLv1.booleanOption('Cortex_M55_1', 'Enable RTSS-HE Cortex-M55 trace', defaultValue=True),
                            DTSLv1.booleanOption('timestamp', "Enable ETM Timestamps", description="Controls the output of timestamps into the ETM output streams", defaultValue=True),
                ...
                +[DTSLv1.tabPage("itm", "ITM", childOptions=[
                    DTSLv1.booleanOption('RTSS-HP ITM-M55', 'Enable RTSS-HP ITM-M55 trace', defaultValue=False),
                ...
                +[DTSLv1.tabPage("sync", "CTI Synchronization", childOptions=[
                    ...
                    DTSLv1.booleanOption(coreNames_cortexA32[0], coreNames_cortexA32[0], description="Add core to synchronization group", defaultValue=False, isDynamic=True),
                    ...
                    DTSLv1.booleanOption(coreNames_cortexM55[1], coreNames_cortexM55[1], description="Add core to synchronization group", defaultValue=False, isDynamic=True),
                ])]
            )
        ]
    ...
    def discoverDevices(self):
        ...
        AHBAP(self, self.findDevice("RTSS-HP AHB-AP"), "RTSS-HP AHB-AP")
        ...
        # Ensure that macrocell StreamIDs are grouped such that they can be filtered by a programmable replicator.
        streamID += (0x10 - (streamID % 0x10))
        for coreName in (coreNames_cortexA32):
            # Create core
            coreDevice = a32_rams.A32CoreDevice(self, self.findDevice(coreName), coreName)
            deviceInfo = DeviceInfo("core", "Cortex-A32")
            coreDevice.setDeviceInfo(deviceInfo)
            self.cortexA32cores.append(coreDevice)
            self.addDeviceInterface(coreDevice)
            a32_rams.registerInternalRAMs(coreDevice)
        ...
        itm = V7M_ITMTraceSource(self, self.findDevice("RTSS-HP ITM-M55"), itmStreamID, "RTSS-HP ITM-M55", self.getDeviceInterfaces().get("RTSS-HP Cortex-M55"))
    ...
    def setInitialOptions(self):
        ...
        coreTraceEnabled = self.getOptionValue("options.cortexA32.coreTrace")
        for core in range(len(coreNames_cortexA32)):
            tmName = self.getTraceSourceNameForCore(coreNames_cortexA32[core])
            if tmName:
                coreTM = self.getDeviceInterface(tmName)
                thisCoreTraceEnabled = self.getOptionValue("options.cortexA32.coreTrace.Cortex_A32_%d" % core)
                enableSource = coreTraceEnabled and thisCoreTraceEnabled
                self.setTraceSourceEnabled(tmName, enableSource)
                if(self.getOptionValue("options.cortexA32.coreTrace.traceRange")):
                    coreTM.clearAllTraceRanges()
                    coreTM.addTraceRange(self.getOptionValue("options.cortexA32.coreTrace.traceRange.start"),
                                         self.getOptionValue("options.cortexA32.coreTrace.traceRange.end"))
                coreTM.setTimestampingEnabled(self.getOptionValue("options.cortexA32.coreTrace.timestamp"))
                self.setContextIDEnabled(coreTM,
                                     self.getOptionValue("options.cortexA32.coreTrace.contextIDs"),
                                     "32")
        ...
        itmEnabled = self.getOptionValue("options.itm.RTSS-HP ITM-M55")
        self.setTraceSourceEnabled("RTSS-HP ITM-M55", itmEnabled)
        ...
        traceModeSet = {self.getOptionValue("options.cortexA32.traceCapture"), self.getOptionValue("options.cortexM55.traceCapture")}
        for traceMode in traceModeSet:
            if traceMode != "none":
                # ETR Capture
                ...
                if traceMode in ["DSTREAM", "DSTREAM_PT_Store_and_Forward", "DSTREAM_PT_StreamingTrace"]:
                    ...
                    if self.getOptions().getOption("options.trace." + self.getDstreamOptionString(traceMode) + ".tpiuPortWidth"):
                        self.setPortWidth(int(self.getOptionValue("options.trace." + self.getDstreamOptionString(traceMode) + ".tpiuPortWidth")))
                    
                    if self.getOptions().getOption("options.trace." + self.getDstreamOptionString(traceMode) + ".traceBufferSize"):
                        self.setTraceBufferSize(self.getOptionValue("options.trace." + self.getDstreamOptionString(traceMode) + ".traceBufferSize"))
    ...
    def updateDynamicOptions(self):
        ...
        for core in range(len(self.cortexA32cores)):
            a32_rams.applyCacheDebug(configuration = self,
                                     optionName = "options.rams.cacheDebug",
                                     device = self.cortexA32cores[core])
            a32_rams.applyCachePreservation(configuration = self,
        ...
        for core in range(len(self.cortexA32cores)):
            enable = self.getOptionValue('options.sync.%s' % coreNames_cortexA32[core])
            self.setCrossSyncEnabled(self.cortexA32cores[core], enable)
    ...
    def getOptionList():
        return [
            DTSLv1.tabSet("options", "Options", childOptions=
                ...
                +[DTSLv1.tabPage("cortexA32", "Cortex-A32", childOptions=[
                    DTSLv1.enumOption('traceCapture', 'Trace capture method', defaultValue="none",
                        values = [("none", "None"), ("APSS TMC", "System Memory Trace Buffer (APSS TMC/ETR)"), ("SoC TMC", "System Memory Trace Buffer (SoC TMC/ETR)"), DtslScript_DSTREAM_ST.getOffChipTraceOption()]),
                    DTSLv1.booleanOption('coreTrace', 'Enable Cortex-A32 core trace', defaultValue=False,
                        childOptions = [
                            DTSLv1.booleanOption('Cortex_A32_0', 'Enable APSS Cortex-A32 trace', defaultValue=True),
                            DTSLv1.booleanOption('timestamp', "Enable ETM Timestamps", description="Controls the output of timestamps into the ETM output streams", defaultValue=True),
    ...
    def getOptionList():
        return [
            DTSLv1.tabSet("options", "Options", childOptions=
                [DTSLv1.tabPage("trace", "Trace Capture", childOptions=[
                    DTSLv1.integerOption('timestampFrequency', 'Timestamp frequency', defaultValue=25000000, isDynamic=False, description="This value will be used to set the Counter Base Frequency ID Register of the Timestamp generator.\nIt represents the number of ticks per second and is used to translate the timestamp value reported into a number of seconds.\nNote that changing this value may not result in a change in the observed frequency."),
                    DTSLv1.infoElement("dpt_storeandforward", "DSTREAM-PT 8GB Trace Buffer", "",
                        childOptions=[
                            DTSLv1.enumOption('tpiuPortWidth', 'TPIU Port Width', defaultValue="4",
                                values = [("1", "1 bit"), ("2", "2 bit"), ("3", "3 bit"), ("4", "4 bit")], isDynamic=False),
                        ]
                    ),
                    DTSLv1.infoElement("dpt_streamingtrace", "DSTREAM-PT Streaming Trace", "",
                        childOptions=[
                            DTSLv1.enumOption('tpiuPortWidth', 'TPIU Port Width', defaultValue="4",
                                values = [("1", "1 bit"), ("2", "2 bit"), ("3", "3 bit"), ("4", "4 bit")], isDynamic=False),
                            DTSLv1.enumOption('traceBufferSize', 'Host Trace Buffer Size', defaultValue="4GB",
                ...
                +[DTSLv1.tabPage("cortexA32", "Cortex-A32", childOptions=[
                    DTSLv1.enumOption('traceCapture', 'Trace capture method', defaultValue="none",
                        values = [("none", "None"), ("APSS TMC", "System Memory Trace Buffer (APSS TMC/ETR)"), ("SoC TMC", "System Memory Trace Buffer (SoC TMC/ETR)"), DtslScript_DSTREAM_PT.getStoreAndForwardOptions(), DtslScript_DSTREAM_PT.getStreamingTraceOptions()]),
                    DTSLv1.booleanOption('coreTrace', 'Enable Cortex-A32 core trace', defaultValue=False,
                        childOptions = [
                            DTSLv1.booleanOption('Cortex_A32_0', 'Enable APSS Cortex-A32 trace', defaultValue=True),
                            DTSLv1.booleanOption('timestamp', "Enable ETM Timestamps", description="Controls the output of timestamps into the ETM output streams", defaultValue=True),
    ...
    def getOptionList():
        return [
            DTSLv1.tabSet("options", "Options", childOptions=
                ...
                +[DTSLv1.tabPage("cortexA32", "Cortex-A32", childOptions=[
                    DTSLv1.enumOption('traceCapture', 'Trace capture method', defaultValue="none",
                        values = [("none", "None"), ("APSS TMC", "System Memory Trace Buffer (APSS TMC/ETR)"), ("SoC TMC", "System Memory Trace Buffer (SoC TMC/ETR)")]),
                    DTSLv1.booleanOption('coreTrace', 'Enable Cortex-A32 core trace', defaultValue=False,
                        childOptions = [
                            DTSLv1.booleanOption('Cortex_A32_0', 'Enable APSS Cortex-A32 trace', defaultValue=True),
                            DTSLv1.booleanOption('timestamp', "Enable ETM Timestamps", description="Controls the output of timestamps into the ETM output streams", defaultValue=True),
